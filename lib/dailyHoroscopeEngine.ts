...
type OpenAIResponsePayload = {
  output_text?: string;
  output?: Array<{ content?: OpenAITextContent[] }>;
...
export async function POST(request: Request) {
  try {
    ...
    const openAIResponse = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        model: process.env.DAILY_HOROSCOPE_MODEL || "gpt-5-mini",
        input: prompt,
    ...
    return NextResponse.json(
      { error: "Today’s horoscope could not be prepared. Please try again." },
      { status: 500 }
    );
  }
}
