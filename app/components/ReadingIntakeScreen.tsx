...
// ── STAR GENERATION ────────────────────────────────────────────────────
// Each star has a fixed position + unique drift/twinkle params computed once.
interface StarData {
  id: number;
  left: number;   // percent
  top: number;    // percent
  ...
  twinkleDelay: number;
  driftX: number; // px — how far it drifts horizontally
  driftY: number; // px — how far it drifts vertically
  driftDuration: number;
  driftDelay: number;
  bright: boolean; // brighter star that pulses more dramatically
}
...
function generateStars(count: number): StarData[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    const r1 = Math.abs(seeded(i * 7 + 1));
    const r2 = Math.abs(seeded(i * 7 + 2));
    const r3 = Math.abs(seeded(i * 7 + 3));
    const r4 = Math.abs(seeded(i * 7 + 4));
    const r5 = Math.abs(seeded(i * 7 + 5));
    const r6 = Math.abs(seeded(i * 7 + 6));
    const r7 = Math.abs(seeded(i * 7 + 7));
    const bright = r7 > 0.88; // ~12% are "bright" stars
    return {
      id: i,
      left: r1 * 100,
      top: r2 * 100,
      size: bright ? 2.5 + r3 * 1.5 : 0.8 + r3 * 1.4,
      opacity: bright ? 0.7 + r4 * 0.3 : 0.2 + r4 * 0.45,
      twinkleDuration: 1.8 + r5 * 3.2,
      twinkleDelay: r6 * 5,
      driftX: (r1 - 0.5) * 18,  // ±9px horizontal drift
      driftY: (r2 - 0.5) * 14,  // ±7px vertical drift
      driftDuration: 14 + r3 * 20, // 14–34s slow float
      driftDelay: r4 * -20,      // stagger start
      bright,
    };
  });
}
  ...
  return (
    ...
    >
      <style jsx>{`
        ...
        /* ── TAP LATENCY FIX ── kills 300ms mobile tap delay on interactive elements */
        .area-btn { touch-action: manipulation; -webkit-tap-highlight-color: transparent; }
        ...
        @keyframes shootingStar {
          ...
          5% { opacity: 1; transform: translateX(0) translateY(0) scaleX(1); }
          100% { transform: translateX(280px) translateY(80px) scaleX(1); opacity: 0; }
        }
        .shooting-star {
          position: absolute;
          width: 80px;
          height: 1px;
          background: linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.9) 50%, rgba(255,255,255,0.4) 100%);
        ...
        /* ── AREA CARD — CSS-only transitions, no Framer animate prop ── */
        .selected-card-shell {
          position: relative;
          overflow: hidden;
          isolation: isolate;
        ...
        .selected-card-shell[data-selected="true"]::before, .selected-card-shell[data-selected="true"]::after { opacity: 1; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .jxl-sparkle, .gold-shimmer::before, .selected-card-shell[data-selected="true"],
      ...
      <div className="pointer-events-none fixed inset-0" aria-hidden="true">
        {/* Shooting star — animates periodically via Framer */}
        {!shouldReduceMotion && (
          <motion.div
            ...
            transition={{
              duration: 2.4,
              times: [0, 0.08, 0.18, 0.72, 1],
              repeat: Infinity,
              repeatDelay: 18 + Math.random() * 14,
              ease: "easeOut",
            }}
          />
        )}
            ...
            animate={
              shouldReduceMotion
                ...
                : {
                    // Twinkle
                    opacity: star.bright
                      ? [star.opacity * 0.3, star.opacity, star.opacity * 0.5, star.opacity * 1.0, star.opacity * 0.3]
                      : [star.opacity * 0.4, star.opacity * 1.5, star.opacity * 0.4],
                    scale: star.bright ? [1, 1.8, 1.1, 1.6, 1] : [1, 1.4, 1],
                    // Slow drift
                    x: [0, star.driftX * 0.5, star.driftX, star.driftX * 0.5, 0],
                    y: [0, star.driftY * 0.3, star.driftY, star.driftY * 0.7, 0],
                  }
            }
            transition={
              shouldReduceMotion
      ...
      >
        ...
        >
          ...
          {onCooldown ? (
                ...
                <div className="cooldown-glow w-full rounded-[28px] border border-indigo-400/20 bg-[#050816]/95 px-6 py-6 text-center backdrop-blur-sm">
                  ...
                  <div className="mt-5 border-t border-white/10 pt-8">
                    ...
                    <button
                      ...
                      className="area-btn rounded-2xl bg-indigo-400 px-6 py-2.5 text-[13px] font-semibold text-slate-950 transition hover:bg-indigo-300 disabled:opacity-60"
                    >
            ...
            <>
              {/* ── AREA BUTTONS — no Framer animate prop, pure CSS transitions ── */}
                ...
                {AREAS.map((area) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "area-btn selected-card-shell standard-shadow w-full rounded-[24px] border px-4 py-4 text-left backdrop-blur-sm",
                    ...
                    >
                      ...
                      <div className="relative z-[1] flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p
                            className="mt-1 text-sm leading-5 transition-colors duration-200"
                            style={{ color: isSelected ? "rgba(241,245,249,0.92)" : "rgba(148,163,184,1)" }}
                          >
                            {area.description}
                          </p>
                        </div>
          ...
          <div className="mt-6 space-y-3 pb-2" ref={clusterBottomRef}>
            ...
            {!onCooldown && (
              <Button
                ...
                className="area-btn standard-shadow h-14 w-full rounded-2xl border border-indigo-400/30 bg-gradient-to-b from-indigo-500 to-indigo-700 text-white text-[15px] font-medium transition-all duration-300 hover:scale-[1.01] disabled:cursor-not-allowed disabled:border-white/10 disabled:bg-slate-900/60 disabled:text-slate-500"
              >
                {buttonCopy}
              </Button>
            )}
                    ...
                    <div className="carousel-content">
                      ...
                      >
                        <div
                          ...
                          style={{ display: "flex", height: "100%", transform: `translateX(-${currentCardIndex * 100}%)`, transition: isDragging || isMouseDragging ? "none" : "transform 0.4s ease" }}
                        >
                      ...
                      {!userStatus?.isSubscribed && (
                        <div className="p-4 pt-3 border-t border-amber-300/10">
                          <button
                              ...
                              (async () => {
                                try {
                                  const res = await fetch("/api/stripe/checkout", {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({ returnUrl: `${window.location.origin}/reading/intake`, mode: "subscription", paywallIndex: 1 }),
                                  });
                            ...
                            className="area-btn h-10 w-full rounded-xl bg-amber-300/20 border border-amber-300/30 text-amber-200 text-[13px] font-semibold transition hover:bg-amber-300/30"
                          >
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15, ease: "easeOut" }} className="mt-4">
            ...
            <div className="relative overflow-hidden rounded-[28px] border border-indigo-400/20 bg-black/30 pointer-events-none select-none" aria-hidden="true">
